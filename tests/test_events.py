...
def _auction(
    ...
    *,
    ...
    return AuctionStateSnapshot(
        ...
        feature_version="auction_measurement_math_v6",
    )
...
def test_alignment_excludes_future_asof_even_when_already_available() -> None:
    auction_as_of = datetime(2024, 3, 4, 10, 5, tzinfo=UTC)
    alignment_time = auction_as_of + timedelta(minutes=30)
    future = _imsi(auction_as_of + timedelta(minutes=5), "future-asof")
    aligner = SnapshotAligner()
    aligner.add_imsi(future)

    synergy = aligner.align(
        replace(_auction(auction_as_of), available_at_utc=alignment_time),
        alignment_time,
    )

    assert synergy.imsi_snapshot_id is None
    assert "IMSI:MISSING" in synergy.blocking_quality_flags


def test_alignment_equal_publication_frontier_prefers_latest_asof_independent_of_insertion() -> (
    None
):
    older_as_of = datetime(2024, 3, 4, 10, tzinfo=UTC)
    newer_as_of = older_as_of + timedelta(minutes=5)
    publication = newer_as_of + timedelta(minutes=5)
    older = replace(_imsi(older_as_of, "older"), available_at_utc=publication)
    newer = replace(_imsi(newer_as_of, "newer"), available_at_utc=publication)

    selected_ids = []
    for snapshots in ((older, newer), (newer, older)):
        aligner = SnapshotAligner()
        for snapshot in snapshots:
            aligner.add_imsi(snapshot)
        selected_ids.append(aligner.align(_auction(publication), publication).imsi_snapshot_id)

    assert selected_ids == ["imsi-newer", "imsi-newer"]
...
def test_candidate_trigger_direction_and_parent_are_semantically_bound() -> None:
    timestamp = datetime(2024, 3, 4, 10, 5, tzinfo=UTC)
    with pytest.raises(DataQualityError, match="direction"):
        EventTrigger(
    ...
    with pytest.raises(DataQualityError, match="parent"):
        EventTrigger(
...
def test_quality_provenance_preserves_information_and_blocks_unready_math() -> None:
    ...
    informational.add_imsi(
        _imsi(
            event_time,
            "info",
            quality_flags=("IMSI_FULL_MODEL_DEFERRED",),
        )
    )
    ...
    assert "IMSI:IMSI_FULL_MODEL_DEFERRED" in ready.component_quality_flags
