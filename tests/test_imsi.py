...
def test_imsi_snapshot_identity_commits_availability_time() -> None:
    base = datetime(2024, 1, 1, 14, 30, tzinfo=UTC)
    first = _bar(0, base=base, close=100.0, volume=10.0, session_id="session-a")
    current = _bar(1, base=base, close=101.0, volume=20.0, session_id="session-a")
    baseline = IMSIStateCore("ES", "ESH24")
    delayed = IMSIStateCore("ES", "ESH24")
    assert baseline.on_bar(first, SessionType.RTH, base) is None
    assert delayed.on_bar(first, SessionType.RTH, base) is None

    baseline_snapshot = baseline.on_bar(current, SessionType.RTH, base)
    delayed_snapshot = delayed.on_bar(
        replace(current, available_at_utc=current.available_at_utc + timedelta(seconds=7)),
        SessionType.RTH,
        base,
    )
    ...
    guarded = IMSIStateCore("ES", "ESH24")
    future_available = replace(
        first,
        available_at_utc=first.available_at_utc + timedelta(days=3),
    )
    guarded.on_bar(future_available, SessionType.RTH, base)
    with pytest.raises(DataTimingInvariantError, match="frontier"):
        guarded.on_bar(current, SessionType.RTH, base)
...
def test_bar_vwap_tod_covariance_neighbors_and_prefix_causality() -> None:
    ...
    for day in range(115):
        ...
        assert "IMSI_FULL_MODEL_DEFERRED" in second.quality_flags
