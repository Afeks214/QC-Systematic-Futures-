...
def test_poc_and_contiguous_value_area_cover_ties_sparse_bins_and_edges() -> None:
    ...
    at = datetime(2024, 2, 15, 15, 0, tzinfo=UTC)
    with pytest.raises(DataQualityError, match="current_price_tick"):
        build_profile_snapshot(
            root="ES",
            ...
            definition=DEFAULT_PROFILE_DEFINITION,
            tick_size=0.25,
            current_price_tick=400.5,  # type: ignore[arg-type]
            volume_by_tick={400: 1.0},
            expected_total_volume=1.0,
        )


@pytest.mark.analytic_math
@pytest.mark.stress_math
def test_profile_math_is_invariant_to_histogram_insertion_order() -> None:
    poc_items = [(1, 1e16), (3, 1e16), (2, 1.0), (4, 1.0), (5, 1.0)]
    poc_first = dict(poc_items)
    poc_reordered = dict(poc_items[index] for index in (0, 4, 1, 2, 3))
    assert poc_first == poc_reordered
    assert select_poc(poc_first) == select_poc(poc_reordered) == 1

    small_bins = [(index + 2, 1e-15) for index in range(200)]
    value_area_first = dict([(0, 70.0), (1, 30.0), *small_bins])
    value_area_reordered = dict([*small_bins, (0, 70.0), (1, 30.0)])
    assert value_area_first == value_area_reordered
    assert select_value_area(value_area_first, 0, 0.70) == select_value_area(
        value_area_reordered,
        0,
        0.70,
    )
...
@pytest.mark.analytic_math
@pytest.mark.stress_math
def test_profile_snapshot_identity_commits_stable_total_volume() -> None:
    at = datetime(2024, 2, 15, 15, 0, tzinfo=UTC)
    small_bins = [(index + 1002, 1e-15) for index in range(200)]
    first = dict([(1000, 80.0), (1001, 20.0), *small_bins])
    reordered = dict([*small_bins, (1000, 80.0), (1001, 20.0)])
    expected = math.fsum(first.values())
    ...
    snapshots = tuple(
        build_profile_snapshot(
            ...
            definition=DEFAULT_PROFILE_DEFINITION,
            tick_size=0.25,
            current_price_tick=1001,
            volume_by_tick=histogram,
            expected_total_volume=expected,
        )
        for histogram in (first, reordered)
    )

    assert snapshots[0] == snapshots[1]
    assert snapshots[0].snapshot_id == snapshots[1].snapshot_id

    flagged = replace(snapshots[0], quality_flags=("DEGENERATE_PROFILE",))
    rebuilt_flagged = build_profile_snapshot(
        root="ES",
        contract_symbol="ESH24",
        session_id="current",
        profile_kind=ProfileKind.DEVELOPING_SESSION,
        ...
        tick_size=0.25,
        current_price_tick=1001,
        volume_by_tick=first,
        expected_total_volume=expected,
        quality_flags=flagged.quality_flags,
    )
    assert rebuilt_flagged.snapshot_id != snapshots[0].snapshot_id


@pytest.mark.analytic_math
@pytest.mark.stress_math
def test_online_profile_aggregation_retains_low_order_volume_independent_of_order() -> None:
    at = datetime(2024, 3, 4, 15, tzinfo=UTC)

    def snapshot_for(quantities: tuple[float, ...]):  # type: ignore[no-untyped-def]
        engine = VolumeProfileEngine("ES", "ESH24", "current", 0.25)
        for index, quantity in enumerate(quantities):
            timestamp = at + timedelta(seconds=index)
            assert engine.ingest_trade(
                TradeObservation(
                    root="ES",
                    contract_symbol="ESH24",
        ...
        return engine.snapshot(ProfileKind.DEVELOPING_SESSION, timestamp, timestamp)

    first = snapshot_for((1e16, 1.0, 1.0))
    reordered = snapshot_for((1.0, 1.0, 1e16))
    assert first.total_volume == reordered.total_volume == math.fsum((1e16, 1.0, 1.0))
    assert first.volume_by_tick == reordered.volume_by_tick
...
def test_profile_snapshot_cannot_precede_processed_observation_availability() -> None:
    exchange_time = datetime(2024, 3, 4, 14, 30, 1, tzinfo=UTC)
    available_at = exchange_time + timedelta(minutes=30)
    engine = VolumeProfileEngine("ES", "ESH24", "current", 0.25)
    engine.ingest_trade(
        TradeObservation(
            root="ES",
    ...
    with pytest.raises(DataTimingInvariantError, match="processed observation"):
        engine.snapshot(
    ...
    with pytest.raises(DataTimingInvariantError, match="frontier"):
        engine.ingest_trade(
            TradeObservation(
                root="ES",
                contract_symbol="ESH24",
                exchange_time_utc=exchange_time + timedelta(minutes=1),
                available_at_utc=exchange_time + timedelta(minutes=1),
                price=100.25,
                quantity=1.0,
                minimum_tick=0.25,
                session_id="current",
...
def test_rejected_profile_observation_advances_availability_frontier() -> None:
    exchange_time = datetime(2024, 3, 4, 14, 30, tzinfo=UTC)
    engine = VolumeProfileEngine("ES", "ESH24", "current", 0.25)
    ...
    def observation(
        ...
        *,
        ...
        return TradeObservation(
            root="ES",
            contract_symbol="ESH24",
            exchange_time_utc=exchange_time + timedelta(seconds=seconds),
    ...
    assert engine.ingest_trade(observation(0, available_at_utc=exchange_time, event_id="admitted"))
    future_frontier = exchange_time + timedelta(days=1)
    assert not engine.ingest_trade(
        observation(
            1,
            available_at_utc=future_frontier,
            event_id="rejected",
            flags=("SOURCE_SUSPICIOUS",),
        )
    )
    with pytest.raises(DataTimingInvariantError, match="frontier"):
        engine.ingest_trade(
            observation(
                2,
                available_at_utc=exchange_time + timedelta(seconds=2),
                event_id="would-backdate",
            )
        )
    with pytest.raises(DataTimingInvariantError, match="processed observation"):
        engine.snapshot(
            ProfileKind.DEVELOPING_SESSION,
            exchange_time + timedelta(seconds=2),
            exchange_time + timedelta(seconds=2),
        )


def test_rejected_profile_observation_advances_asof_frontier() -> None:
    exchange_time = datetime(2024, 3, 4, 14, 30, tzinfo=UTC)
    available_at = exchange_time + timedelta(hours=1)
    engine = VolumeProfileEngine("ES", "ESH24", "current", 0.25)
    ...
    assert engine.ingest_trade(observation(0, "admitted"))
    assert not engine.ingest_trade(observation(40, "rejected", flags=("SOURCE_SUSPICIOUS",)))
    assert engine.ingest_trade(observation(35, "delayed-admitted"))
    with pytest.raises(DataTimingInvariantError, match="processed observations"):
        engine.snapshot(
            ProfileKind.DEVELOPING_SESSION,
            exchange_time + timedelta(seconds=35),
            available_at,
        )


def test_cached_final_profile_cannot_be_returned_before_its_clocks() -> None:
    exchange_time = datetime(2024, 3, 4, 14, 30, tzinfo=UTC)
    engine = VolumeProfileEngine("ES", "ESH24", "current", 0.25)
    assert engine.ingest_trade(
        TradeObservation(
            ...
            contract_symbol="ESH24",
            exchange_time_utc=exchange_time,
            available_at_utc=exchange_time,
            price=100.0,
            quantity=1.0,
            minimum_tick=0.25,
            session_id="current",
    ...
    final_time = exchange_time + timedelta(hours=1)
    final = engine.snapshot(ProfileKind.FINAL_SESSION, final_time, final_time)

    with pytest.raises(DataTimingInvariantError, match="cached final"):
        engine.snapshot(
            ProfileKind.FINAL_SESSION,
            exchange_time + timedelta(minutes=1),
            exchange_time + timedelta(minutes=1),
        )
    assert (
...
def test_auction_features_require_exact_bar_atr_and_transition_identity() -> None:
    ...
    with pytest.raises(DataTimingInvariantError, match="prior Profile is unavailable"):
        auction_features(
            current,
            replace(prior, available_at_utc=at + timedelta(days=1)),
            atr,
            bars,
            transitions,
        )
    with pytest.raises(DataQualityError, match="ATR version"):
        auction_features(
            current,
            prior,
            replace(atr, version="unsupported-atr-semantics"),
            bars,
            transitions,
        )
...
def test_measurement_quality_severity_is_exact_and_unknown_dedup_is_blocking() -> None:
    assert (
        measurement_quality_severity("PROVENANCE:DEDUPLICATION_UNVERIFIABLE")
        is MeasurementQualitySeverity.BLOCKING
    )
    ...
    assert blocking_measurement_flags(("PROVENANCE:DEDUPLICATION_UNVERIFIABLE",)) == (
        "PROVENANCE:DEDUPLICATION_UNVERIFIABLE",
    )
