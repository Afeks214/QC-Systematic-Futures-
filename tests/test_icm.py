...
def test_icm_snapshot_identity_commits_availability_time() -> None:
    baseline = ICMEngine("ES", "ESH24")
    delayed = ICMEngine("ES", "ESH24")
    baseline_snapshot = delayed_snapshot = None
    for index in range(70):
        bar = _bar(index, 100.0 + 0.02 * index + 0.1 * np.sin(index))
        baseline_snapshot = baseline.on_bar(bar)
    ...
    with pytest.raises(DataTimingInvariantError, match="frontier"):
        guarded.on_bar(_bar(1, 100.25))


def test_icm_rejects_unsupported_atr_version_before_state_mutation() -> None:
    engine = ICMEngine("ES", "ESH24")
    bar = _bar(0, 100.0)
    unsupported = ATRMeasurement(
        root="ES",
        ...
        version="unsupported-atr-semantics",
    )

    with pytest.raises(DataQualityError, match="ATR version"):
        engine.on_bar(bar, unsupported)
...
def test_icm_local_scale_distance_and_residual_autocorrelation_match_oracles() -> None:
    ...
    engine = ICMEngine("ES", "ESH24")
    final_bar = _bar(69, float(prices[-1]))
    atr = ATRMeasurement(
        root="ES",
        contract_symbol="ESH24",
        as_of_utc=final_bar.end_utc,
        available_at_utc=final_bar.available_at_utc,
        value=0.5,
        observation_count=24,
        warmup_complete=True,
    ...
    snapshot = None
    for index, price in enumerate(prices):
        bar = _bar(index, float(price))
        snapshot = engine.on_bar(bar, atr if index == 69 else None)
    assert snapshot is not None
